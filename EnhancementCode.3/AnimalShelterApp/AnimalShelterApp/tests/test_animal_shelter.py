...
def main():
    ...
    try:
        ...
        adoptable_dogs = shelter.read_with_filters(animal_type="Dog", outcome_type="Adoption")
        print(f"Adoptable dogs found: {len(adoptable_dogs)}")
        for animal in adoptable_dogs:
            print(f"- {animal.get('name')} ({animal.get('animal_type')} / {animal.get('outcome_type')})")

        #Test sorting filters
        print("\n=== Sorting Tests ===")

        sorted_by_name = shelter.read_all_sorted_by_name()
        print("Animals sorted by name:")
        for animal in sorted_by_name:
            print(f"- {animal.get('name')}")

        sorted_by_age = shelter.read_all_sorted_by_age()
        print("\nAnimals sorted by age:")
        for animal in sorted_by_age:
            print(f"- {animal.get('name')} ({animal.get('age_upon_outcome_in_weeks')} weeks)")

        #Bad insert to test the validation
        print("\n=== Validation Test ===")
        ...
        result = shelter.create(bad_animal)
        print(f"Invalid insert result: {result}")
